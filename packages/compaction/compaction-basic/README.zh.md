...
- **溢出恢复**：提供方已确认的溢出会绕过常规压力与保留，执行剪枝，然后以有界且保持工具调用／结果配对平衡的分片压缩最旧内容：每个分片按已路由模型公布的上下文窗口确定大小，使其回放（系统提示词／工具／前缀、该区域、追加的指令、受 `maxTokens` 限制的摘要以及 `overflowChunkHeadroomTokens`）保持在窗口以下，并重复恢复直到表层能容纳在窗口内。路由未公布容量，或该窗口无法容纳哪怕一个有界分片时，则回退为一次最大平衡头部缩减，并留下最新不可分单元。只要 `surface.replaceGeneration` 前进，就允许重试，包括剪枝在后续摘要工作抛出异常前已落地的情况。如果没有替换、目标特定上限已耗尽、已取消，或遇到未知／非规范错误，则保留原始提供方失败。
...
| `overflowChunkHeadroomTokens` | 否（默认 `512`） | 为每个溢出恢复分片确定大小时，除回放标头、指令和 `maxTokens` 外额外预留的 token；使其摘要请求保持在模型窗口以下。 |
...
适配器可能无法为有效动态路由返回容量，已解析容量也可能暴露无效的绝对保留预算。此时手动压力检查会抛出目标特定配置错误；自动 listener 会对该精确目标警告一次，并携带完整历史继续。不相关的操作性失败仍会独立可见。规范提供方溢出仍会尝试恢复：它以有界分片压缩最旧内容（每个分片都预留窗口内空间，用于回放标头、追加的指令、受 `maxTokens` 限制的摘要和 `overflowChunkHeadroomTokens`）；路由未公布可用于确定分片大小的容量时，则回退为整个表层压缩。

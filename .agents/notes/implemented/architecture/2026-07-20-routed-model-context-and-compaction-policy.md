...
An adapter that lacks capacity metadata remains a valid LLM route. Manual proactive pressure fails with a target-specific configuration error; the automatic listener warns once per exact route and continues with full history. The same per-route suppression applies when resolved capacity exposes an invalid absolute retention budget, while unrelated operational failures remain independently visible. Canonical provider-confirmed overflow bypasses the proactive threshold and normal retention budget; it resolves the routed model's capacity when available and compacts the oldest content in bounded, tool-pair-balanced chunks below the window budget, falling back to a whole-surface reduction only when the route advertises no capacity. Either path preserves the original provider error unless replacement proves progress. The [bounded overflow compaction chunks Agent Note](2026-12-01-bounded-overflow-compaction-chunks.md) supersedes the earlier "one maximal balanced reduction / no capacity metadata" overflow decision.

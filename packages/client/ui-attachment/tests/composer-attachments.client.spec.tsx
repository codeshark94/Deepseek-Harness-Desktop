...
import type {
  ComposerAttachment, ComposerAttachmentsOwnerProps, ComposerAttachmentsProps, ComposerFileAttachment,
...
describe('ComposerAttachments', () => {
  ...
  it('renders uploaded files beside image drafts and routes their removal by occurrence id', () => {
    const onRemoveFile = vi.fn()
    const file: ComposerFileAttachment = { occurrenceId: 7, name: 'report.pdf' }
    const view = render(<ComposerAttachments {...props({ files: [file], onRemoveFile })} />)
    expect(view.getByText('report.pdf')).toBeTruthy()
    fireEvent.click(view.getByRole('button', { name: '移除文件 report.pdf' }))
    expect(onRemoveFile).toHaveBeenCalledWith(7)
  })

...
/** Rail item retaining its source reference for the relevant callback. */
type ComposerRailItem =
  | (Extract<AttachmentRailItem, { kind: 'image' }> & { readonly attachment: ComposerAttachment })
  | (Extract<AttachmentRailItem, { kind: 'file' }> & { readonly occurrenceId: number })

/** Draft-image rail, document drop target, and original-image preview slot entry. */
export function ComposerAttachments({
  attachments, files, canAcceptDrop, onAddImages, onRemoveImage, onRemoveFile, dropLimits, t,
  ...
  const railItems = useMemo<ComposerRailItem[]>(() => [
    ...
    ...files.map(file => ({
      kind: 'file' as const,
      id: `file-${file.occurrenceId}`,
      name: file.name,
      removeLabel: t('file.remove', { name: file.name }),
        ...
        <div className={css.rail}>
          <AttachmentRail
            ...
            onRemove={(item) => {
              ...
              else onRemoveFile(item.occurrenceId)
            }}
          />
        </div>
      )}

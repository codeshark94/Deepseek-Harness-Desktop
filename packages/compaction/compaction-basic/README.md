...
- **Overflow recovery** — provider-confirmed overflow bypasses normal pressure and retention, prunes, then compacts the oldest content in bounded, tool-pair-balanced chunks: each chunk is sized from the routed model's advertised context window so its replay (system/tools/prefix, the region, the appended instruction, and the `maxTokens`-capped summary, plus `overflowChunkHeadroomTokens`) stays below the window, and recovery repeats until the surface fits. When the route advertises no capacity or the window cannot host even one bounded chunk, it falls back to one maximal balanced head reduction while leaving the newest indivisible unit. Retry is authorized whenever `surface.replaceGeneration` advances, including when pruning lands before later summary work throws. No replacement, an exhausted target-specific cap, cancellation, or an unknown/noncanonical error preserves the original provider failure.
...
| `overflowChunkHeadroomTokens` | no (default `512`) | Extra tokens kept free, beyond the replay header, instruction, and `maxTokens`, when sizing each overflow-recovery chunk so its summarization request stays below the model window. |

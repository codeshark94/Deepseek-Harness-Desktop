...
describe('decorations', () => {
  ...
  it('attaching a file uploads it and inserts a chip with the filename, not the raw content', async () => {
    const { view, shell, slotCalls } = bench()
    ...
    expect(attachmentOwner(slotCalls).files).toEqual([{ occurrenceId: occurrence.occurrenceId, name: 'report.pdf' }])
  })
  ...
  it('removing a file card drops its chip and occurrence from the draft', async () => {
    ...
    expect(shell.snapshot.occurrences).toHaveLength(1)
    const occurrence = shell.snapshot.occurrences[0]!
    act(() => { attachmentOwner(slotCalls).onRemoveFile(occurrence.occurrenceId) })
    expect(shell.snapshot.occurrences).toEqual([])
    expect(shell.snapshot.draft).not.toContain('\uFFFC')
    expect(attachmentOwner(slotCalls).files).toEqual([])
  })

...
export function InputBar({
  ...
  // File attachment: insert the picked file as a reference chip at the caret.
  // The draft shows only the filename chip; on submit the message carries a
  // reference to the file, not its content. The model reads the file from the
  // workspace with tool-fs when it is available.
  ...
  const removeFile = (occurrenceId: number): void => {
    ...
    const draft = input.draft
    inputActions.setDraft(draft.slice(0, occurrence.offset) + draft.slice(occurrence.offset + occurrence.length))
  }

  const files = useMemo(
    () => (input?.occurrences ?? [])
      .filter(occurrence => occurrence.source === 'file')
      .map(occurrence => ({ occurrenceId: occurrence.occurrenceId, name: occurrence.label })),
    [input?.occurrences],
  )
